...
function($scope, $http, $document, $stateParams, $uibModal, $timeout )
{
	...
	var paramList = JSON.parse(param["paramList"]);
		...
		return new Promise(function(resolve, reject)
        {
			...
			$http.post('./api/loadTemplate.php',fd,{
				...
				if( data.result == "OK" )
				{
                    ...
                    var dcaseInfo = data.template;
                    var root = data.template["root"];
					
                    for( var key in data.template.partsList )
                    {
                        var parts = data.template.partsList[key];
						console.log("parts",parts, viewPort );
						parts.x = parts.x - root.x + svgTagWidth/2;
						parts.y = parts.y - root.y + root.height;
                        partsList[parts.id] = parts;
						for(idx in paramList)
						{
							var keyWord = paramList[idx];
							parts.detail = parts.detail.replace("${"+keyWord.key+"}", keyWord.value);
						}
                    }
					root.x = svgTagWidth/2;
					root.y = root.height;
					for(idx in paramList)
					{
						var keyWord = paramList[idx];
						root.detail = root.detail.replace("${"+keyWord.key+"}", keyWord.value);
					}
					partsList[root.id] = root;
	...
	function loadData(date, authUserInfo)
	{
		...
		loadDCase(date).then(function(ret_data)
		{
			data = ret_data;
			console.log("loadData", ret_data);
